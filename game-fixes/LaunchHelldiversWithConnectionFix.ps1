...
# Issue: It seems that Helldivers 2 (And some other games) doesn't respect the default MTU (Maximum Transmission Unit) for Windows. Reaffirming this setting before launching the game
...
# Finally, make sure the $gamePathway variable is the location of the launch .exe for Helldivers 2.
...
$gamePathway = "F:\SteamLibrary\steamapps\common\Helldivers 2\bin\helldivers2.exe"
...
$isdAmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
